...
export const AppSplashIntro: React.FC<AppSplashIntroProps> = ({ onComplete }) => {
  ...
  return (
    <div
      className={`fixed inset-0 z-50 flex flex-col items-center justify-center bg-slate-50 text-slate-900 transition-opacity duration-500 selection:bg-orange-500 selection:text-white ${
        isFading ? 'opacity-0 pointer-events-none' : 'opacity-100'
      }`}
    >
      {/* Background Radial Glow */}
      <div className="absolute top-1/3 left-1/2 -translate-x-1/2 -translate-y-1/2 w-96 h-96 bg-orange-400/10 rounded-full filter blur-3xl pointer-events-none animate-pulse" />
      <div className="absolute bottom-1/4 right-1/4 w-80 h-80 bg-sky-500/10 rounded-full filter blur-3xl pointer-events-none" />
      ...
      <div className="relative z-10 flex flex-col items-center text-center px-6 space-y-6 max-w-sm">
        ...
        <div className="w-48 h-1.5 bg-slate-200 rounded-full overflow-hidden border border-slate-300/60 shadow-inner mt-4">
          <div className="h-full bg-gradient-to-r from-orange-500 via-amber-400 to-sky-500 animate-pulse rounded-full w-full" />
        </div>
...
export const AppOutroModal: React.FC<AppOutroModalProps> = ({
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-900/60 backdrop-blur-sm animate-fade-in">
      <div className="relative w-full max-w-md bg-white border border-slate-200 rounded-3xl p-6 shadow-2xl space-y-5 text-center text-slate-900 overflow-hidden">
        {/* Glow */}
        <div className="absolute top-0 right-0 w-40 h-40 bg-orange-500/5 rounded-full filter blur-2xl pointer-events-none" />
        ...
        <button
          onClick={onClose}
          className="w-full bg-orange-500 hover:bg-orange-600 text-white font-bold py-3 rounded-2xl shadow-md transition-all text-sm shadow-orange-500/20"
        >
          እሺ (Close)
        </button>
      </div>
